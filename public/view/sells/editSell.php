    ...
    <div class="container py-5">
        ...
        <div class="w-50 mt-2">
            <form method="POST" action="/sell/edit">
                ...
                <input type="hidden" name="id" value="<?php echo htmlspecialchars($_GET['id'] ?? ($data['formData']['id'] ?? '')); ?>">
                <div class="mb-3">
                        ...
                        <select class="form-select" id="produto" name="produto">
                            ...
                            if (is_array($produtos) && count($produtos) > 0) {
                                foreach ($produtos as $produto) {
                                    $isSelected = (isset($data['formData']['produto']) && $data['formData']['produto'] == $produto['id']) ? 'selected' : '';
                                    echo "<option value=\"{$produto['id']}\" {$isSelected}>" . htmlspecialchars($produto['nome']) . "</option>";
                                }
                            }
                            ?>
                        </select>
